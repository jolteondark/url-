...
export function applySafariBattlePhaseUi() {
  ...
  for (const button of document.querySelectorAll?.("button[data-bag-use-item]") ?? []) {
    ...
    button.inert = !commandAllowed;
  }

  const card = byId("battle-card");
  if (card) {
    card.dataset.battlePhase = phase ?? "UNSYNCED";
    card.dataset.turnPhaseLocked = commandAllowed || resultReady ? "false" : "true";
    card.setAttribute("aria-busy", String(!commandAllowed && !resultReady));
  }
}
