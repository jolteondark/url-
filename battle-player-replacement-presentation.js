...
function replacementOptions(battle) {
  return Array.isArray(battle?.player_replacement_options) ? battle.player_replacement_options : [];
}
...
async function chooseReplacement(button) {
  ...
  const legal = replacementOptions(battle).some((option) => Number(option?.partyIndex) === partyIndex);
  if (!legal) return;
  selecting = true;
  syncReplacementUi();
  try {
