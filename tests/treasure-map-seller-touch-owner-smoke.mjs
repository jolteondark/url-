...
assert.match(touch, /safariTreasureMapSellerPresentation/);
assert.match(touch, /resolveSafariTreasureMapSellerAccusation/);
assert.match(touch, /resolveSafariTreasureMapSellerInteraction/);
assert.match(touch, /persistSafariOwnerResult/);
assert.match(touch, /normal_event_trainer_battle_started/);
assert.doesNotMatch(touch, /confirm\s*\(/);
assert.doesNotMatch(touch, /saveSafariPlayableRun/);

console.log("treasure map seller touch owner smoke: ok");
