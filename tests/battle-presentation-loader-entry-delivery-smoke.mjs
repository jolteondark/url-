...
assert.match(
  uiBridge,
  /canonicalBattleUiAssetUrl\("cursor_command\.png"\)/,
  "reachable Battle UI must retain the canonical command cursor consumer covered by this public generation",
);
assert.doesNotMatch(
  index,
  /deferred-ui-loader\.js\?v=20260902-1006/,
  "stale pre-command-cursor deferred loader pin must not return",
);
assert.doesNotMatch(
  index,
...
console.log(`ok - Battle presentation outer loader pin ${revision} covers nested status ${nestedStatusRevision} and command cursor delivery`);
