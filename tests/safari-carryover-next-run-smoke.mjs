...
assert.match(uiSource, /export async function renderSafariCarryoverSelection\(\)/,
  "carryover selector must expose a deterministic render entrypoint");
assert.match(uiSource, /safari-preview-start/,
  "selector must still react to preview-start restore");
assert.match(uiSource, /持ち越し候補を読み込めませんでした。通常スターターなら次のランを開始できます。/,
  "candidate load failure must leave an explicit canonical fallback instead of a blank Run End screen");
assert.match(previewSource, /carryover-next-run-presentation\.js\?v=20260818-1552/,
  "preview must load the rescue selector build");
assert.match(previewSource, /await carryoverPresentation\.renderSafariCarryoverSelection\?\.\(\)/,
  "preview must explicitly render selector after synchronous restore dispatch");
assert.match(previewSource, /carryoverPanelVisible/,
  "boot trace must expose whether the selector actually appeared");
assert.match(indexSource, /preview\.js\?v=20260818-1552/,
  "public entrypoint must expose the rescue preview build");

console.log("Safari carryover Run End -> deterministic selector/fallback -> next Day Board: ok");
