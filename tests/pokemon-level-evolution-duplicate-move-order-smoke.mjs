...
const speciesMasters = {
  ...
  TARGET: {
    ...
    base_stats: baseStats,
    evolutions: [],
    // Essentials builds moves_to_learn in source order. If the same move is
    // listed for both level 0 and the current level, declining the first offer
    // must not suppress the second offer.
...
assert.equal(resolverCalls, 2, "duplicate canonical evolution move entries must be offered in source order until the move is actually learned");
assert.deepEqual(result.pokemon.moves.map((move) => move.id), ["A", "DUPMOVE", "C", "D"]);
assert.equal(result.pokemon.moves[0].pp, 3, "retained move PP must not refill");
assert.equal(result.pokemon.moves[2].pp, 5, "retained move PP must not refill");
assert.equal(result.pokemon.moves[3].pp, 6, "retained move PP must not refill");
...
assert.deepEqual(
  ...
  "first decline and second canonical offer must remain distinguishable",
);
