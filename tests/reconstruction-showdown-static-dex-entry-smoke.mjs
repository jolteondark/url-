...
const rendered = renderStaticDexEntryModule({
  ...
  support: [
    { scope: 'support', file: 'config/formats.ts', absolute: '/pinned/config/formats.ts' },
  ],
  mods: [
...
assert.match(rendered, /\/pinned\/config\/formats\.ts/);
assert.match(rendered, /support: Object\.freeze/);
