...
assert.equal(egg.species, "NICKIT");
...
assert.equal(state.day, 18);
assert.equal(restoredEgg.species, "NICKIT");
...
assert.equal(runtime.player.party.at(-1).species, "NICKIT");
assert.equal(runtime.player.party.at(-1).mapless_egg_shop_bonus_pending, true);
assert.equal(Number(runtime.bag.money), moneyBefore - MAPLESS_EGG_SHOP_PRICE_V108);

console.log("Safari DAY18 egg shop -> purchase -> 20-visit Egg -> Save/Continue -> DAY19: PASS");
