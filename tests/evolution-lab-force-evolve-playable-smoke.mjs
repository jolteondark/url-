...
const bulbasaur = {
  species:"BULBASAUR", level:5, personal_id:122, gender:0, form:0,
  moves:[{ id:"TACKLE", pp:30, ppup:0 }], exp:135, hp:20, status:null, status_count:0,
  item:null, ability_id:"OVERGROW", ability_index:0, nature_id:"HARDY", nature_for_stats_id:"HARDY",
  ready_to_evolve:true, iv:{ ...ZERO }, ev:{ ...ZERO }, max_hp:20,
  stats:{ ATTACK:10, DEFENSE:10, SPECIAL_ATTACK:10, SPECIAL_DEFENSE:10, SPEED:10 },
};

const hydrated = resolveEvolutionLabForceEvolutionContextV108(bulbasaur, { op:"force_evolve", pokemon_index:0, species:"IVYSAUR" });
...
assert.deepEqual(hydrated.base_stats, { HP:60, ATTACK:62, DEFENSE:63, SPECIAL_ATTACK:80, SPECIAL_DEFENSE:80, SPEED:60 });
...
for (let seed = 0; seed < 10000; seed += 1) {
  const owner = resolveCanonicalEvolutionLabV108({
    ...
    choice:"stable", party:[bulbasaur], selected_index:0,
  });
...
assert.ok(runtime.player.party[0].stats.SPECIAL_ATTACK > runtime.player.party[0].stats.SPEED);
