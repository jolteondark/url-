...
import {
  MAPLESS_DYNAMIC_TRAINER_ARCHETYPES,
  MAPLESS_DYNAMIC_TRAINER_NAMES,
  MAPLESS_DYNAMIC_TRAINER_PARTY_SIZE,
  MAPLESS_DYNAMIC_TRAINER_PRIZE,
  MAPLESS_DYNAMIC_TRAINER_SKILL,
} from "./mapless-dynamic-trainer-master.js";

export { MAPLESS_DYNAMIC_TRAINER_ARCHETYPES } from "./mapless-dynamic-trainer-master.js";
...
export function generateSafariDynamicTrainer({ day, rank = "NORMAL", extraModifier = 0, partySize = null, randomValues = [] } = {}) {
  ...
  const count = partySize == null
    ? MAPLESS_DYNAMIC_TRAINER_PARTY_SIZE.minimum + pick(MAPLESS_DYNAMIC_TRAINER_PARTY_SIZE.maximum - MAPLESS_DYNAMIC_TRAINER_PARTY_SIZE.minimum + 1)
    : Math.max(MAPLESS_DYNAMIC_TRAINER_PARTY_SIZE.minimum, Math.min(MAPLESS_DYNAMIC_TRAINER_PARTY_SIZE.maximum, Math.trunc(Number(partySize))));
  ...
  const namePool = archetype.gender === 0 ? MAPLESS_DYNAMIC_TRAINER_NAMES.male : archetype.gender === 1 ? MAPLESS_DYNAMIC_TRAINER_NAMES.female : MAPLESS_DYNAMIC_TRAINER_NAMES.unknown;
