...
async function startWild(runtime, event, index, operations) {
  await ensureSafariGeneralData();
  const { resolveSafariGeneralEncounter } = await import("./safari-general-encounter-runtime.js");
...
async function startTrainer(runtime, event, index, operations) {
  await ensureSafariGeneralData();
  const { generateSafariDynamicTrainer } = await import("./mapless-dynamic-trainer-generator.js");
...
async function activateCombatCell(runtime, index, event) {
  const state = stateOf(runtime);
  await ensureSafariGeneralData();
  ...
  if (dispatch.result === "dispatched") { if (event.kind === "wild") await startWild(runtime, event, index, dispatch.operations); else await startTrainer(runtime, event, index, dispatch.operations); }
