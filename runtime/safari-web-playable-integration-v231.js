...
function recordFullRuntimeError(runtime, error) {
  globalThis.__maplessLastError = error;
  const state = stateOf(runtime);
  if (state.battle && !state.battle.completed) {
    state.notice = `戦闘処理の読み込みに失敗しました: ${error?.message ?? error}`;
    notifySafariRuntimeChanged();
  }
}

function warmBattleRuntime(runtime) {
  full()
    .then(() => notifySafariRuntimeChanged())
    .catch((error) => recordFullRuntimeError(runtime, error));
}
...
export async function activateSafariDayBoardCell(runtime, index) {
  ...
  if (event?.kind === "wild" || event?.kind === "trainer") {
    try {
      ...
      if (stateOf(runtime).battle && !stateOf(runtime).battle.completed) {
        queueMicrotask(() => warmBattleRuntime(runtime));
      }
      return result;
    } catch (error) {
      globalThis.__maplessLastError = error;
...
export async function resolveSafariBattleRound(runtime, selectedMoveId) {
  try {
    ...
    recordFullRuntimeError(runtime, error);
    throw error;
  }
}
