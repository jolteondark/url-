...
function generalPockets(slots = []) {
  return { general: { slots, maxSlots: 20, maxPerSlot: 99 } };
}

function rewardMeta(ids) {
  return Object.fromEntries(Array.from(new Set(ids)).map((id) => [
    id,
    { valid: hasMaplessV108ItemMetadata(id), pocket: "general" },
  ]));
}
...
export function resolveHoneyTreeHoneyGrantV108({ slots = [], count = 1 } = {}) {
  ...
  return resolveRewardTransaction({
    pockets: generalPockets(slots),
    itemMeta: rewardMeta(items),
    items,
  });
}
...
export function resolveHoneyTreeActionRewardV108({ event, action, slots = [], sharedRandomInt } = {}) {
  ...
  const localRng = new RubyMT19937Random(seed);

  if (action === "bug") {
    if (hasMaplessV108ItemMetadata("HONEY")) {
      const honeyCount = 2 + localRng.randInt(2);
      const reward = resolveHoneyTreeHoneyGrantV108({ slots, count: honeyCount });
    ...
    const itemMeta = rewardMeta(MAPLESS_NORMAL_EVENT_SMALL_REWARD_ITEMS);
    const reward = resolveMaplessNormalEventSmallReward({
      count: 2,
      randomInt: sharedRandomInt,
      pockets: generalPockets(slots),
      itemMeta,
    });
  ...
  if (roll < 50) {
    ...
    const pool = owned.length > 0
      ...
      : MAPLESS_HONEY_TREE_FALLBACK_BERRIES_V108.filter(hasMaplessV108ItemMetadata);
    if (pool.length === 0) return { reward: null, selectedItems: [], honeyCount: 0, operations: [] };
    const item = pool[localRng.randInt(pool.length)];
    const reward = resolveRewardTransaction({
      pockets: generalPockets(slots),
      itemMeta: rewardMeta([item]),
      items: [item],
    });
