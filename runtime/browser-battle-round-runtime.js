...
import {
  ...
  materializeBattleParty,
  prepareBrowserPartyAwareJudgeStates,
} from "./browser-battle-party-judge.js";
...
function resolveRoundMove({ pokemon, selectedMoveId, moveMasters, label, autoStruggle = false }) {
  ...
  if (struggle) {
    ...
    return { moveIndex: -1, move: STRUGGLE_MOVE_CANONICAL, struggle: true, runtimeMove: null };
  }
  ...
  return { moveIndex, move, struggle: false, runtimeMove };
}
function actionInput({ actor, target, move, moveIndex, battlerIndex, targetBattlerIndex, randomRoll, reflectPp, struggle = false }) {
  const special = move.category === "Special";
  const action = {
    kind: "move", battlerIndex, targetBattlerIndex, actorHpBefore: actor.hp, actorTotalHp: actor.max_hp,
    moveIndex, moveId: move.id, accuracyInput: { baseAccuracy: move.accuracy, randomRoll },
    damageInput: { level: actor.level, baseDamage: move.power, attack: actor.stats[special ? "SPECIAL_ATTACK" : "ATTACK"], defense: target.stats[special ? "SPECIAL_DEFENSE" : "DEFENSE"], attackStageIndex: 6, defenseStageIndex: 6 },
...
export function resolveBrowserBattleRound({ player, foe, playerParty = null, foeParty = null, playerActivePartyIndex = 0, foeActivePartyIndex = 0, selectedMoveId, foeMoveId, moveMasters, playerRandomRoll = 0, foeRandomRoll = 0, postBattlePersistenceInput = null, reflectedPartyIndex = 0 } = {}) {
  requireBattleStats(player, "player"); requireBattleStats(foe, "foe");
  const playerPartyState = materializeBattleParty(playerParty, playerActivePartyIndex, player, "player");
  const foePartyState = materializeBattleParty(foeParty, foeActivePartyIndex, foe, "foe");
  ...
  const round = {
    attackPhaseInput: { battlers: [{ battlerIndex: 0, choiceKind: "UseMove", fainted: player.hp <= 0, choseRageFunction: false }, { battlerIndex: 1, choiceKind: "UseMove", fainted: foe.hp <= 0, choseRageFunction: false }] },
    commandEntries: [commandEntry(0, true, playerResolved, 1), commandEntry(1, false, foeResolved, 0)],
    priorityEntries: [{ actionIndex: 0, battlerIndex: 0, speed: player.stats.SPEED, movePriority: playerMove.priority, tieBreaker: 1 }, { actionIndex: 1, battlerIndex: 1, speed: foe.stats.SPEED, movePriority: foeMove.priority, tieBreaker: 0 }],
    actions: [actionInput({ actor: player, target: foe, move: playerMove, moveIndex: playerMoveIndex, battlerIndex: 0, targetBattlerIndex: 1, randomRoll: playerRandomRoll, reflectPp: !playerResolved.struggle, struggle: playerResolved.struggle }), actionInput({ actor: foe, target: player, move: foeMove, moveIndex: foeMoveIndex, battlerIndex: 1, targetBattlerIndex: 0, randomRoll: foeRandomRoll, reflectPp: !foeResolved.struggle, struggle: foeResolved.struggle })],
  };
  const battleInput = { useAttackPhaseScheduler: true, useCanonicalAccuracyDamage: true, rounds: [round] };
  ...
  const scheduling = playerRuntime.attackPhaseScheduling; const combat = playerRuntime.combatTrace; const preparedRound = combat.rounds[0]; const decision = Number(playerRuntime.turn.decision);
  const operations = presentationOperations(playerRuntime.turn.operations, preparedRound, selection); const playerPp = playerRuntime.battlePpIntegration ?? { prepared: [], commits: [] }; const foePp = foeRuntime.battlePpIntegration ?? { prepared: [], commits: [] }; const postBattlePersistence = playerRuntime.postBattlePersistence ?? null;
  const battleContinuationHandoff = buildBrowserBattleContinuationHandoff({ playerParty: playerPartyState.party, foeParty: foePartyState.party, playerPartyIndex: playerPartyState.activePartyIndex, foePartyIndex: foePartyState.activePartyIndex, playerPokemon: playerRuntime.pokemon, foePokemon: foeRuntime.pokemon, decision });
  return { player: playerRuntime.pokemon, foe: foeRuntime.pokemon, decision, operations, selection, scheduling, struggle: { player: playerResolved.struggle, foe: foeResolved.struggle }, ppIntegration: { prepared: playerPp.prepared, commits: [...playerPp.commits.map((commit) => ({ ...commit, actor: "player" })), ...foePp.commits.map((commit) => ({ ...commit, actor: "foe" }))] }, battleContinuationHandoff, battleResultHandoff: { ...playerRuntime.battleResultHandoff, postBattlePersistence }, battleRuntimeIntegration: { start: playerRuntime.start, combatTrace: combat, awaitingNextRound: Boolean(playerRuntime.turn.awaitingNextRound), playerPpCommits: playerPp.commits.length, foePpCommits: foePp.commits.length, postBattlePersistenceApplied: postBattlePersistence !== null, partyAwareJudge: true } };
}
