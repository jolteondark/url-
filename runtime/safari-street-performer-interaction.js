...
function persistenceRequestedFromOperations(operations) {
...
function commitResolvedEvent(runtime, index, owner, appliedOperations) {
  ...
  if (!persistenceRequestedFromOperations(operations)) {
    operations.push({ op:"request_save", reason:"normal_event_resolved" });
  }
...
registerSafariNormalEventBattleContinuation("street_performer", (runtime, continuation) => {
  ...
  return {
    ...
    notice:state.notice,
    persistenceRequested:persistenceRequestedFromOperations(state.last_operations),
    owner,
  };
});
...
export async function resolveSafariStreetPerformerInteraction(runtime, index, requestedAction) {
  ...
  if (raw.startsWith("perform:")) {
    ...
    return { runtime, result:owner.outcome, completed:true, reward, optionalReward, operations:state.last_operations, notice:state.notice, persistenceRequested:persistenceRequestedFromOperations(state.last_operations), owner };
  }
  ...
  if (raw === "watch") {
    ...
    return { runtime, result:owner.outcome, completed:true, viewingPrice, operations:state.last_operations, notice:state.notice, persistenceRequested:persistenceRequestedFromOperations(state.last_operations), owner };
  }
  ...
  if (raw === "callout") {
    ...
    return { runtime, result:owner.outcome, completed:true, operations:state.last_operations, notice:state.notice, persistenceRequested:persistenceRequestedFromOperations(state.last_operations), owner };
  }
