...
export function pokemonAbleCanonical(pokemon) {
...
export function allFaintedCanonical(party = []) {
  return !(Array.isArray(party) ? party : []).some(pokemonAbleCanonical);
}

export function resolveJudgeCanonical(input = {}) {
  const hasParties = Array.isArray(input.playerParty) && Array.isArray(input.foeParty);
  const playerAllFainted = hasParties ? allFaintedCanonical(input.playerParty) : Boolean(input.playerAllFainted);
  const foeAllFainted = hasParties ? allFaintedCanonical(input.foeParty) : Boolean(input.foeAllFainted);
  let decision = 0;
  ...
  return { decision, playerAllFainted, foeAllFainted, sourceComplete: hasParties, sourceSymbol: "Battle#pbJudge", sourceBodySha256: JUDGE_BODY_SHA256, ableCountBodySha256: ABLE_COUNT_BODY_SHA256, allFaintedBodySha256: ALL_FAINTED_BODY_SHA256, pokemonAbleBodySha256: POKEMON_ABLE_BODY_SHA256 };
}
